...
public:
	...
	/**
	 * @fn	virtual void FHoloPlayViewportClient::ProcessScreenShots(FViewport* Viewport) override;
	 *
...
private:
	...
	/**
	 * @fn	bool FHoloPlayViewportClient::HandleScalabilityCommand(const TCHAR* Cmd, FOutputDevice& Ar);
	 *
	 * @brief	Handles the Scalability command
	 *
	 * @param 		  	Cmd	The command.
	 * @param [in,out]	Ar 	The archive.
	 *
	...
	bool HandleScalabilityCommand(const TCHAR* Cmd, FOutputDevice& Ar);
	...
	/**
	 * @fn	void FHoloPlayViewportClient::DrawDebugParameters(FCanvas* Canvas);
	 *
	 * @brief	Draw debug parameters from screen calibration to FCanvas
	 *
	 * @param [in,out]	Canvas	If non-null, the canvas.
	 */

	void DrawDebugParameters(FCanvas* Canvas);
	...
	/**
	 * @fn	static bool FHoloPlayViewportClient::ParseResolution(const TCHAR* InResolution, uint32& OutX, uint32& OutY, EHoloPlayWindowType& OutWindowMode);
	 *
	 ...
	 * @param [in,out]	OutWindowMode	The out window mode.
	 *
	 * @returns	True if it succeeds, false if it fails.
	 */

	static bool ParseResolution(const TCHAR* InResolution, uint32& OutX, uint32& OutY, EHoloPlayWindowType& OutWindowMode);
	...
	void ProcessScreenshot2D(TWeakObjectPtr<AHoloPlayCapture> HoloPlayCapture);

	/* Draw Debug Text to Screen*/

	/**
	 * @fn	void FHoloPlayViewportClient::DrawDebugText(FCanvas * InCanvas, FVector Location, FHoloPlayConfigValues Value, FColor Color, float Scale);
	 *
	 * @brief	Draw debug text of calibration data to screen
	 *
	 * @param [in,out]	InCanvas	If non-null, the in canvas.
	...
	void DrawDebugText(FCanvas * InCanvas, FVector Location, FHoloPlayConfigValues Value, FColor Color, float Scale);

	/**
	 * @fn	TWeakObjectPtr<AHoloPlayCapture> FHoloPlayViewportClient::GetGameHoloPlayCapture();
	 *
	 * @brief	Gets game holo play capture settings actor
	 *
	 * @returns	The game holo play capture.
	 */

	TWeakObjectPtr<AHoloPlayCapture> GetGameHoloPlayCapture();

	/**
	 * @fn	UTextureRenderTarget2D* FHoloPlayViewportClient::GetQuiltRT(TWeakObjectPtr<AHoloPlayCapture> HoloPlayCapture);
	 *
	 * @brief	Gets quilt render target texture
	 *
	 * @param	HoloPlayCapture	The holo play capture.
	 *
	 * @returns	Null if it fails, else the quilt right.
	 */

	UTextureRenderTarget2D* GetQuiltRT(TWeakObjectPtr<AHoloPlayCapture> HoloPlayCapture);
